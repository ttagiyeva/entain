...
func (u *userRepoTestSuite) TestCreateTransaction() {
	...
	u.EqualError(err, fmt.Sprintf("repo.user.GetUser: %v", model.ErrorNotFound))
	u.Nil(us)
}
...
func (u *userRepoTestSuite) TestUpdateUserBalance() {
	...
	user.Balance = -100
	err = u.repo.UpdateUserBalance(tx, ctx, user)
	u.EqualError(err, fmt.Sprintf("repo.user.UpdateUserBalance: %v", model.ErrorInsufficientBalance))
